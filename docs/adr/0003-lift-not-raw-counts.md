...
## Consequences
A laptop sleeve — rarely viewed alone, almost always viewed with a laptop —
has low count and high lift, and now ranks where it should. Lift is undefined
for a product with no interactions yet; the API returns `null` and flags
`score_undefined` rather than silently substituting zero.
