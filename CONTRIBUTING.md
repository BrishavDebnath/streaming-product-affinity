...
This is a personal portfolio project, not a library with users, so it does
not need a governance document. What it does need is for anyone reading it,
including me in six months, to be able to change it without breaking the
properties it claims. This file is for that.
...
pytest                                # 128 tests, no Kafka or MongoDB needed
...
- Anything stateful in Spark must be windowed. A `groupBy` on business
  keys alone retains state for every key ever seen, and the watermark cannot
  evict it. `tests/test_transforms.py` asserts state stays bounded, in a real
  streaming query.
